...
Open **Data delivery** for each candidate's onboard queue, contact-limited transmissions, processing delays and per-product delivery outcomes. Existing missions use **Propose delivery inputs** after approving coverage inputs (reopen a baseline first). Review and approve the illustrative delays, then recalculate. **Configure latency requirement** opens the requirement: explicitly set the maximum acquisition-to-delivery metric, `<=`, and a threshold such as `30 minute` if that is the intended requirement. Review affected design content when prompted. Prose alone does not set the deadline. Dropped or overdue products fail; pending, truncated or absent observations cannot produce a full-workload pass. See the [delivery model and limits](docs/architecture/delivery-analysis.md).
...
Editable inputs cover both candidates' mass entries/allocations, power loads/mode fractions/solar/battery assumptions, data rate/duty/compression/storage, RF/link/contact inputs, and the common orbit altitude. Period, eclipse and downlink demand are derived by tools and cannot be overridden. Original compatible units (including percent) are retained. Missing/invalid input groups block recalculation; calculator failures record Unknown results and block selection.
...
Playwright uses installed Google Chrome by default; set `CHROME_PATH` if needed. Its configuration starts isolated servers on ports 5174/8011 and migrates a separate temporary browser-test database, leaving the normal app on ports 5173/8000 untouched. Tests use separate SQLite databases; the browser acceptance test creates a new uniquely named mission.
...
GitHub Actions runs lint/format checks, SQLite and migrated PostgreSQL regression suites, the frontend build/tests, and both Playwright workflows on pushes and pull requests. The workflow has read-only repository permissions and uses no paid model credentials.
