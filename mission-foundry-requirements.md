...
| MF-REQ-011 | A requirement shall optionally carry a structured quantitative criterion consisting of a supported measured output, an inclusive upper or lower comparison, and a finite nonnegative threshold with compatible units. Compatible units shall be converted explicitly; incompatible units and unsupported metrics shall be rejected. | Must | Test |
| MF-REQ-012 | The system shall evaluate each quantitative criterion separately for each architecture candidate using a recorded deterministic analysis output. Each result shall link to the requirement and available evidence and record the criterion, actual value and unit, evaluation revision, evidence revision, and analysis source revision. | Must | Test |
| MF-REQ-013 | Requirement check status shall be separate from requirement approval and shall be pass, fail, stale, or unverified. Missing, failed, or incompatible evidence shall be unverified. Changes to a requirement or its dependent calculation inputs shall invalidate existing evidence; recalculation shall refresh the check. | Must | Test |
| MF-REQ-014 | A failing or stale must-priority criterion shall block candidate selection and baseline approval. Conceptual approval may retain unverified requirements and failed lower-priority criteria only as explicitly recorded outstanding obligations, without asserting mission feasibility. | Must | Test |
| MF-REQ-015 | Users shall be able to add, revise, or remove a quantitative criterion through the reviewed requirement-edit workflow, inspect per-candidate results and evidence, and retain prior checks in immutable baseline exports. Removing a criterion shall leave the requirement unverified. | Must | Demonstration |

Quantitative verification scope for version 0.2: the initial metric registry covers allocated mass, average power load, orbit energy margin, eclipse battery margin, daily data generation, daily downlink capacity, orbital period, and eclipse duration. A criterion applies to both reference candidates. The owner must ensure that the chosen metric actually verifies the requirement statement; requirement prose is not automatically interpreted or rewritten. Daily capacity does not verify acquisition-to-user latency, mass does not verify a deployer envelope, and preliminary calculations do not establish flight qualification. Existing requirements receive no inferred numerical criteria. Signed calculated margins are permitted; the supported requirement thresholds are nonnegative. The registry can grow as validated calculators become available.
...
| MF-ANL-024 | The workbench shall show reference/trial outcomes, source revision and limitations and allow JSON export containing trial inputs and calculation evidence. Unsaved results shall clear on changed settings, mission or revision; stale responses shall not appear as current studies. The UI shall disclose that unsaved studies are temporary until saved or exported. | Must | Demonstration |
| MF-ANL-025 | The system shall allow named sensitivity studies to be saved as immutable, mission-scoped records containing calculation evidence, request settings, creation time, source revision and baseline identity. Saving shall recompute evidence from the specified current source rather than trust client-supplied results, and shall reject stale revisions and archived missions. | Must | Test |
| MF-ANL-026 | Saved studies shall survive application reloads and remain available after source-design changes or mission archiving. Saving shall not change mission revisions, approved inputs, verification conclusions or baseline contents. A versioned database migration shall preserve existing mission data and enforce append-only study storage. | Must | Test |
| MF-ANL-027 | The workbench shall retrieve and compare saved studies, label historical source revisions, expose source settings and unknown/invalid outcomes, warn when source designs or study settings differ, and export saved evidence as JSON. Delayed responses shall not populate another mission's view. | Must | Demonstration |
| MF-ANL-028 | The system shall allow a valid saved sensitivity trial to create a design-change proposal for its single varied input. The trial quantity and parameter shall be resolved from mission-scoped stored evidence, not supplied by the caller. Equivalent unchanged values, invalid trials, stale revisions, pending proposals, archived missions and immutable baselines shall be rejected. | Must | Test |
| MF-ANL-029 | Trial proposals shall require current valid delivery evidence and unchanged recorded calculation inputs relative to the study reference. Baseline reopening alone may preserve eligibility. Changed assumptions shall require a new study. Proposals shall record source-study identity, original source revision, trial index, candidate, parameter and before/after values. | Must | Test |
| MF-ANL-030 | The workbench shall preview the current and proposed input, collect a rationale and submit the change through existing review controls. Accepted inputs shall remain unchanged until approval. Approval shall invalidate dependent evidence and require recalculation; rejection shall preserve the design. Saved study evidence and historical baselines shall remain immutable, and trial deadline outcomes shall not be copied as approved verification results. | Must | Demonstration |
